...
function createT_team() {
    ...
    try {
        ...
        var i = 0;
        var id = db.getNext('T_TEAM_ID');
        statement.setInt(++i, id);
        statement.setString(++i, message.name);
        statement.setString(++i, message.leader);
...
function readT_teamEntity(id) {
    ...
    try {
        var result = "";
        var sql = "SELECT * FROM T_TEAM WHERE "+pkToSQL();
        var statement = connection.prepareStatement(sql);
...
function createResultEntity(resultSet, data) {
    ...
    result.leader = resultSet.getString("LEADER");
...
function deleteT_team(id) {
    var connection = datasource.getConnection();
    try {
        var sql = "DELETE FROM T_TEAM WHERE "+pkToSQL();
        var statement = connection.prepareStatement(sql);
        statement.setString(1, id);
...
function countT_team() {
    ...
    try {
        ...
        var rs = statement.executeQuery('SELECT COUNT(*) FROM T_TEAM');
        while (rs.next()) {
            count = rs.getInt(1);
        }
    } catch(e){
...
function metadataT_team() {
	...
	var propertyleader = {};
	propertyleader.name = 'leader';
    propertyleader.type = 'string';
    entityMetadata.properties.push(propertyleader);
